...
import { Banknote, BriefcaseBusiness, CalendarClock, GraduationCap, MapPin, UsersRound } from "lucide-react";
import { ApplyForm } from "@/components/ApplyForm";
import { LogoBadge } from "@/components/LogoBadge";
import { SaveJobButton } from "@/components/SaveJobButton";
import { getJobWithCompany, jobs } from "@/lib/data";
...
export default async function JobDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const details = [
    ...
    { label: "Education", value: job.education, icon: GraduationCap },
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-ink/60">
                  {job.employmentType} / {job.workMode} / {job.category}
                </p>
              </div>
            </div>
          ...
          <section className="panel p-6">
            ...
            <div className="mt-4 flex gap-4">
              ...
              <div>
                ...
                <p className="mt-3 leading-6 text-ink/70">{job.company.description}</p>
                <Link href={job.company.website} className="mt-3 inline-flex text-sm font-bold text-mint">
                  {job.company.website}
                </Link>
              </div>
            </div>
